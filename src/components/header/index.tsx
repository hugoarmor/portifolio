...
function Header(props: Componentprops) {
    ...
    const NavbarElement = (propsNav: {element: string, href: string, onClick?: () => void}) => (
        <Styled.NavbarElement onClick={propsNav.onClick} style={{color: props.colorView ? "#ffffff": "#25291C", transition:"color 2s"}} href={propsNav.href}>{propsNav.element}</Styled.NavbarElement>
    );
